...
func (h *SubcategoriesHTTPHandler) DeleteSubcategory(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	err = h.subcategoriesService.DeleteSubcategory(ctx, subcategoryID, claims.UserID)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
