...
func (h *CategoriesHTTPHandler) GetCategory(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	categoryDomain, err := h.categoriesService.GetCategory(ctx, categoryID, claims.UserID)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
	...
	response := GetCategoryResponse(categoryResponseFromDomain(categoryDomain))
